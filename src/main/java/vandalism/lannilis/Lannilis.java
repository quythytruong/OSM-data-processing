...
public class Lannilis {
	...
	public static void main(String[] args) throws Exception {
		...
		b.addID2Postgres("lannilis", geometries.keySet());
		...
		for (IFeature feat : geometries.values()) {
			...
			query.append("UPDATE indicators.lannilis SET v_contrib =" + version + ", uid = " + uid);
		...
		for (Long bati : geometries.keySet()) {
			...
			query.append("UPDATE indicators.lannilis SET n_inter_lulc=" + nbIntersects + ", n_is_within_lulc="
		...
		for (Long bati : geometries.keySet()) {
			...
			for (IFeature road : roads) {
				...
				query.append("UPDATE indicators.lannilis SET n_inter_route=" + nbIntersects + " WHERE id = "
						+ feat.getId() + ";");
				it++;
				if (it == 1000) {
					b.updatePostgresIndicators(query.toString());

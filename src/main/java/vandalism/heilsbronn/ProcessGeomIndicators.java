...
public class ProcessGeomIndicators {
	public static void main(String[] args) throws Exception {
		String url = "jdbc:postgresql://localhost:5432/heilsbronn";
		Connection conn = DriverManager.getConnection(url, "postgres", "postgres");
		System.out.println("Connexion réussie !");

		Statement s = conn.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
		ResultSet r = s.executeQuery(
				"SELECT id, ST_AsText(ST_TRANSFORM(ST_GeometryN(geom, 1),31467)) as wkt FROM heilsbronn_vandalisme;");

		Map<Integer, IGeometry> vandalized = new HashMap<Integer, IGeometry>();
		StringBuffer query = new StringBuffer();
		query.append("INSERT INTO indicators.heilsbronn_v (id) VALUES ");
		...
		for (int id : vandalized.keySet()) {
			...
			query.append("UPDATE indicators.heilsbronn_v SET perimeter=" + perimeter + ", area =" + area

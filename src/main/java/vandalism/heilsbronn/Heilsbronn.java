...
public class Heilsbronn {
	public static void main(String[] args) throws Exception {

		BuildingAssessment b = new BuildingAssessment("localhost", "5432", "heilsbronn", "postgres", "postgres");
		b.loadBuildings("Heilsbronn", "2018-02-13T23:59:59Z");
		String epsg = "31467";
		Map<Long, IFeature> geometries = b.buildGeometry(epsg);

		b.addID2Postgres("Heilsbronn", geometries.keySet());
		...
		for (IFeature feat : geometries.values()) {
			...
			query.append("UPDATE indicators.heilsbronn SET v_contrib =" + version + ", uid = " + uid);
		...
		for (Long bati : geometries.keySet()) {
			...
			query.append("UPDATE indicators.heilsbronn SET n_inter_lulc=" + nbIntersects + ", n_is_within_lulc="
		...
		for (Long bati : geometries.keySet()) {
			...
			for (IFeature road : roads) {
				...
				query.append("UPDATE indicators.heilsbronn SET n_inter_route=" + nbIntersects + " WHERE id = "
						+ feat.getId() + ";");
				it++;
				if (it == 1000) {
					b.updatePostgresIndicators(query.toString());

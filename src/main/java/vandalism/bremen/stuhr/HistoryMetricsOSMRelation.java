...
public class HistoryMetricsOSMRelation {
	...
	public static void main(String[] args) throws Exception {
		...
		String select = "SELECT id, v_contrib FROM indicators.stuhr WHERE is_way IS FALSE;";
		...
		for (Long id : idRelations.keySet()) {
			...
			for (int v = 1; v <= idRelations.get(id); v++) {
				...
				OsmRelationMember.dbName = "bremen";
			...
			String update = "UPDATE indicators.stuhr SET n_versions=" + nbVersions + ", n_users=" + nbUsers + ", n_del="
					+ nbDeletes + ", n_geom_edit=" + nbGeomEdit + ", n_tag_edit=" + nbTagEdit + ", n_tag_add="
					+ nbTagAdd + ", n_tag_del=" + nbTagDel + " WHERE id = " + id + ";";
			...
			update = "UPDATE indicators.stuhr SET is_del_prev_version = " + isGeomEdit + ", is_tag_edit_prev_version = "
					+ isTagModif + ",is_tag_add_prev_version =" + isTagCreation + ",is_tag_del_prev_version = "
					+ isTagDel + " WHERE id = " + id + ";";
			ld.executeAnyQuery(update);

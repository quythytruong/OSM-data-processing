...
public class ProcessGeomIndicators {
	public static void main(String[] args) throws Exception {
		...
		ResultSet r = s.executeQuery(
				"SELECT id, ST_AsText(ST_TRANSFORM(ST_GeometryN(geom, 1),31467)) as wkt FROM vandalized_building_stuhr;");

		Map<Integer, IGeometry> vandalized = new HashMap<Integer, IGeometry>();
		StringBuffer query = new StringBuffer();
		query.append("INSERT INTO indicators.stuhr_v (id) VALUES ");
		...
		for (int id : vandalized.keySet()) {
			...
			query.append("UPDATE indicators.stuhr_v SET perimeter=" + perimeter + ", area =" + area
